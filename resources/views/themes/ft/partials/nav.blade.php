            ...
            @if($menu->id == 2 || $menu->id == 3)
                <div class="card">
                    <div class=card-header id=heading-desktop-{{ $menu->id }}>
                        <h5 class=mb-0>
                            <a href="{{ url($menu->link) }}" @if(str_contains($route, $menu->link)) class="active" @endif>{{ $menu->title }}</a>
                        ...
                        <div id=collapse{{ $menu->id }} class=collapse aria-labelledby=heading{{ $menu->id }} data-parent=#accordion>
                            <div class=card-body>
                                @foreach($submenu as $sub)
                                    <p> <a href="{{ url($sub->link) }}" @if(str_contains($route, $sub->link)) class="active" @endif>{{ $sub->title }}</a> </p>
                                @endforeach
                            </div>
                        </div>
                    @endif
